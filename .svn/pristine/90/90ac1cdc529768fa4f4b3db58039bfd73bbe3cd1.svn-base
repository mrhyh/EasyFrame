...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    for (int i = 0; i < _listArray.count; i++) {
        ...
        if (i == 0 ) {
            NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:kCategoryCacheKey];
            if(dict != nil){
            ...
            NSDictionary *dict = [[NSUserDefaults standardUserDefaults] objectForKey:kSortCacheKey];
            if(dict != nil){
                [button setTitle:dict[@"name"] forState:UIControlStateNormal];
...
//    }
    ...
    _m_view = [[EFSelectView alloc]initWithFrame:CGRectMake(0, rect.origin.y + rect.size.height, self.view.width, 160) ObjectId:(int)sender.tag-100 Array:nil andSelectBlock:^(CategoryModel *category) {
        _m_view.hidden = YES;
        _m_coverView.hidden = YES;
