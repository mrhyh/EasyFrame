...
- (instancetype)initWithType:(NSString*)_type CallBack:(CompleteBlock)callBack{
    self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    if (self) {
        self.completeBlock = callBack;
        self.Type = _type;
        self.isShowWalletPay = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row != 0) {
        ...
        cell.imageView.image = Img(self.imgArray[indexPath.row-1]);
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        UIAlertView * alert1 = [[UIAlertView alloc]initWithTitle:@"" message:self.Type delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alert1 show];
    }
}
