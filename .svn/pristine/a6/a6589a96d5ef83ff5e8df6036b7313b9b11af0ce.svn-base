...
- (void)EFReloadDataWithModel:(ShopCartContentModel*)model {
    ...
    if(model.productItem.image != nil) {
         //self.efImageView.image = model.productItem.image;
    }
   
    self.nameLabel.text = model.productItem.name;
    self.priceLabel.text = [NSString stringWithFormat:@"￥%ld",(long)model.productItem.price];
