...
- (void)setupView{
    ...
    if (self.isNeedConsigneeInfo == YES) {
        ...
        userAddressLabel.text = [NSString stringWithFormat:@"%@",self.efMallConsigneeModel.address];
...
#warning  TODO 临时测试数据
    ...
    [self.viewModel createOrder:_efMallConsigneeModel.objectId ConsigneeName:_efMallConsigneeModel.consignee ConsigneeMobile:_efMallConsigneeModel.phone ConsigneeAddress:_efMallConsigneeModel.address ConsigneeZipCode:_efMallConsigneeModel.zipCode isInvoice:YES invoiceTitle:_productDetail.name orderProductId:orderProductIdString orderProductCount:orderProductCountString shippingMethodId:0 notes:@"备注"];
}
    ...
    EFPayAlertView *payView = [[EFPayAlertView alloc] initWithType:@"" CallBack:^(NSString *Type) {
        if ([Type isEqualToString:@"我的钱包"]) {
            ...
            [paymentView EFMailcompleteHandle:^(NSString *inputPwd) {
                ...
                NSString *orderIdString = [NSString stringWithFormat:@"%ld",(long)_orderModel.objectId];
                [weakSelf.viewModel PayOrder:orderIdString callBackBlock:nil];
            }];
            [paymentView show];
        }
    }];
    ...
    if (action & EFMailShopCart_CreateOrder) {
        if ([result.jsonDict[@"status"] intValue] == 200) {
            ...
            if (self.viewModel.orderModelArray.count > 0 ){
                _orderModel = self.viewModel.orderModelArray[0];
            }
