...
- (void)viewDidLoad {
    ...
    _isStock = [[[NSDictionary alloc] initWithContentsOfFile:self.plistPath][@"Mall"][@"isStock"] boolValue];
    if (_isStore) {
        _count = 4;
        if (!_isStock) {
            _count = 3;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        return cell;
    }else if(indexPath.section == 1&&_isStock) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    WS(weakSelf)
    if (indexPath.section == 1&&_isStock) {
